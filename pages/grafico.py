...
def _get_ppm_vals():
    ...
    h2, ch4 = [], []
    for i in range(n):
        try:    h2.append(float(st.session_state.get(f"h2_{i}", "")))
        except: h2.append(None)
        try:    ch4.append(float(st.session_state.get(f"ch4_{i}", "")))
        except: ch4.append(None)
...
def _build_chart(h2_vals, ch4_vals, tiempos, umbral, tls) -> bytes:
    ...
    if h2x:
        ax.plot(h2x, h2y, "o-", color=H2_COLOR,  lw=2.2, ms=7, label="H₂",  zorder=3)
        ax.fill_between(h2x, h2y, alpha=0.08, color=H2_COLOR)
    if ch4x:
        ax.plot(ch4x, ch4y, "s-", color=CH4_COLOR, lw=2.2, ms=7, label="CH₄", zorder=3)
...
def render():
    ...
    if any(v is not None for v in h2_vals + ch4_vals):
        ...
        st.info("Ingresá valores PPM en la pestaña **Datos y valores** para ver el gráfico.")
